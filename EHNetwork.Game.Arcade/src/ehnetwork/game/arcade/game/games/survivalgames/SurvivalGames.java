...
public abstract class SurvivalGames extends Game
{
	...
	public void BlockBreak(BlockBreakEvent event)
	{
		...
		if (block.getType().name().contains("CHEST"))
		{
			...
			{
				BlockPosition loc1 = new BlockPosition(block.getX(), block.getY(), block.getZ());
				if (tileEntityChest.getPosition() == loc1)
				{
					WorldServer world = ((CraftWorld) block.getWorld()).getHandle();

					world.playBlockAction(loc1,
							net.minecraft.server.v1_8_R3.Block.getById(block.getTypeId()), 1, 10);
		...
		if (holder instanceof Chest)
		{
			...
			if (tileEntity instanceof TileEntityChest)
			{
				...
				try
				{
					...
					int key = (chest.getPosition().getX() + chest.getPosition().getY() + chest.getPosition().getZ()) % 200;
	...
	public void chestTickEvent(UpdateEvent event)
	{
		...
		while (itel.hasNext())
		{
			...
			try
			{
				int key = (entry.getKey().getPosition().getX() + entry.getKey().getPosition().getY() + entry.getKey().getPosition().getZ()) % 200;
