...
public class SeismicSlam extends SkillActive
{
	...
	public SeismicSlam(SkillFactory skills, String name, IPvpClass.ClassType classType, SkillType skillType,
					   int cost, int levels,
	...
	public void Slam(UpdateEvent event)
	{
		...
		for (Player player : GetUsers())
		{
			...
			//Event
			UtilServer.getServer().getPluginManager().callEvent(new SkillEvent(player, GetName(), IPvpClass.ClassType.Brute, targets.keySet()));
